...
The suite writes timestamped CSV files to `benchmark/results/` and figures to
`benchmark/figures/`.
...
| Real scaled latency | `scaled_latency_real_*.csv` | `latency_vs_nodes.png` | Same detection-latency metric, but the script changes the actual number of running LAN node containers before each topology. |
...
6. Regenerates the four figures.
...
Regenerate figures from the latest CSVs:
...
Latest kept scaled result:

```text
benchmark/results/scaled_latency_real_20260605T015104Z.csv
```

Latest kept end-to-end proof:

```text
benchmark/results/e2e_20260605T022801Z.txt
```

Each topology used 30 trials. The combined CSV has 120 rows total.
...
Each measurement used that exact number of running `lan-node-*` containers.
